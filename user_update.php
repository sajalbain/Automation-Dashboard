...
if ($_GET['corpid'] && $_SESSION['login_user'] && $_GET['corpid'] == $_SESSION['login_user']) {
  $sql = "select firstname,lastname,corpid,role,active,password from users where corpid = '$corpid'";
  mysqli_query($db, $sql);
  $result = mysqli_query($db, $sql);
  $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
} else if ($_GET['corpid'] && $_SESSION['login_user'] && $_GET['corpid'] != $_SESSION['login_user'] && $_SESSION['role'] == 'admin') {
  $sql = "select firstname,lastname,corpid,role,active,password from users where corpid = '$corpid'";
  mysqli_query($db, $sql);
  $result = mysqli_query($db, $sql);
  $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
} else {
    echo '<script>alert("Please contact admin! You don\'t have privledge to access this page"); window.location.href="/users.php";</script>';
}
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($_POST['roles'] == "admin" && $row['role'] == "user") {
    $userlogin = $_SESSION['login_user'];
    $sql1 = "select role from users where corpid = '$userlogin'";
    mysqli_query($db, $sql);
  ...
  if (strlen($password) == 32) {
    $sql = "update users set password='$password',firstname=('$firstname'),lastname='$lastname',role='$role',active='$active' where corpid='$corpid'";
  } else {
    $sql = "update users set password=md5('$password'),firstname=('$firstname'),lastname='$lastname',role='$role',active='$active' where corpid='$corpid'";
  }
  mysqli_query($db, $sql);
  header('location: users.php');
}
// function active($corpid, $active)
// {
//   $sql = "update users set active='$active' where corpid='$corpid'";
// }
?>
            ...
            <div class="col-lg-6 ">
              <div class="card card-info">
                <div class="card-header">
                  ...
                  <a href="/user_delete.php?corpid=<?php echo $row['corpid'] ?>" class='btn btn-danger btn-xs' style="float:right;margin:5px;">Remove User</a>
                </div>
                <form action="" method="post">
                  <div class="card-body">
                    <div class="form-group">
                      <label for="CORPID">CORP ID</label>
                      <input type="text" name="corpid" class="form-control" value="<?php echo $row['corpid'] ?>" required readonly>
                    </div>
                    ...
                    <div class="form-group col-lg-1">
                      ...
                      <?php if ($row['active'] == 1) { ?>
                        <input type="checkbox" value="1" class="form-control" checked>
                      <?php } else { ?>
                        <input type="checkbox" value="0" class="form-control">
                      <?php } ?>
                    </div>
                    <div class="form-group">
                      <input type="number" class="form-control" name="active" value="<?php echo $row['active'] ?>" min="0" max="1" required>
                    </div>
                    <div class="form-group">
